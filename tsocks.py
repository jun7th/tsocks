...
def main():
  ...
  openssl req -new -x509 -keyout cert.pem -out cert.pem -days 1095 -nodes''',
	...
	parser.add_argument('-f','--forward',nargs=2, metavar=('PORT_1', 'PORT_2'),type=int,help='Set forward mode,server connect port_1,client connect port_2')
	parser.add_argument('-d','--debug',action="store_true", default=False,help='Set debug mode,will show debug information')
	parser.add_argument('-S','--ssl',action="store_true", default=False,help='Set use ssl,just support reverse proxy mode,relay server must also use ssl')
	parser.add_argument('-c','--cert',metavar='CERT_FILE', type=str,default="cert.pem",help='Set ssl cert file path,only set relay server')
